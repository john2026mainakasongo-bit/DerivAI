...
function slope(a) {
  ...
  let num = 0, den = 0;
  a.forEach((y, i) => { num += (i - xm) * (y - ym); den += (i - xm) ** 2; });
  return den ? num / den : 0;
}
function atr(a, p = 14) {
  if (a.length < 3) return std(a.slice(-20));
  const d = a.slice(-p - 1).slice(1).map((x, i) => Math.abs(x - a.slice(-p - 1)[i]));
  return mean(d);
}
...
export function analyzeRiseFallTouch(input = []) {
  ...
  if (prices.length < 24) {
    return {
      ready: false, signal: "WAIT", family: "NONE", confidence: 0,
      probability: 0, reason: `Collecting data ${prices.length}/24`,
  ...
  const volatility = atr(prices, 14);
  const deviation = std(prices.slice(-30)) || volatility || 1e-9;
  ...
  const z = (current - mean30) / Math.max(deviation, 1e-9);

  const slopeNorm = trendSlope / Math.max(volatility, 1e-9);
  const trendNorm = (fast - slow) / Math.max(volatility, 1e-9);
  const momentumNorm = momentum / Math.max(volatility, 1e-9);
  ...
  rise = clamp(rise); fall = clamp(fall);

  const trendDirection = rise >= fall ? "RISE" : "FALL";
  const directionConfidence = clamp(50 + Math.abs(rise - fall) * .78);

  const distance = Math.abs(z);
  const trendPressure = Math.min(1.5, Math.abs(trendNorm));
  const touchProbability = clamp(56 + Math.min(20, distance * 7) + trendPressure * 8);
  const noTouchProbability = clamp(100 - touchProbability + (trendPressure > .8 ? 8 : 0));
  const touchSignal = touchProbability >= noTouchProbability ? "TOUCH" : "NO TOUCH";
  const touchConfidence = clamp(50 + Math.abs(touchProbability - noTouchProbability) * .9);

  const barrierSide = trendDirection === "RISE" ? 1 : -1;
  const barrierDistance = Math.max(volatility * .8, deviation * .7);
  ...
  const candidates = [
    {
      ...
      probability: Math.max(rise, fall),
    ...
    },
    {
      ...
      setup: touchSignal,
      probability: Math.max(touchProbability, noTouchProbability),
    ...
    ...x,
    score: clamp(x.confidence * .72 + x.probability * .18 + Math.min(100, prices.length / 80 * 100) * .10)
  })).sort((a, b) => b.score - a.score);

  const best = candidates[0];
  return {
    ready: true,
    signal: best.score >= 72 ? best.setup : "WAIT",
    family: best.family,
