...
export default function DigitOverRecoveryBot() {
  ...
  const currentDigits = useMemo(() => {
    ...
    const previous = Array.isArray(buffers[symbol])
      ? buffers[symbol]
      : [];
  ...
  const empty = useMemo(
    () => ({
      ...
      reason: "Waiting for the 60-digit model to initialize.",
    }),
    [currentDigits]
  );
  ...
  /*
   * FIX:
   * Match the four requested Volatility markets using:
   * - R_100 / R_75 / R_25 / R_10
   * - 1HZ100V / 1HZ75V / 1HZ25V / 1HZ10V
   * - Deriv labels such as:
   *   Volatility 100 Index
   *   Volatility 100 (1s) Index
   *
   * This avoids relying only on an exact label ending.
   */
    ...
    return wanted
      .map((value) => {
        const found = markets.find((item) => {
          ...
          const marketSymbol = String(item?.symbol || "").toUpperCase();
          const label = String(item?.label || "");

          return (
            id === `R_${value}` ||
            id === `1HZ${value}V` ||
            marketSymbol === `R_${value}` ||
            marketSymbol === `1HZ${value}V` ||
            new RegExp(
  ...
  const marketAnalyses = useMemo(
    () =>
      targetMarkets.map((marketItem) => {
        const digits = Array.isArray(
          digitHistoryBySymbol[marketItem.id]
        )
      ...
      marketAnalyses
        .flatMap((item) =>
          item.candidates.map((candidate) => ({
            ...
            market: item.market,
            latestEpoch: Number(
              marketTicks[item.symbol]?.at?.(-1)?.epoch || 0
            ),
        ...
        .sort((a, b) => {
          const rank = (x) =>
            ...
            Number(x.probability || 0) * 0.4 +
            Number(x.stability || 0) * 0.1 +
  ...
  const preferredCandidate =
    allCandidates.find((candidate) => candidate.signal !== "WAIT") ||
    allCandidates[0] || {
      ...empty,
      symbol: symbol || "",
      market: market,
  ...
  const active = openContracts.find(
  ...
  const usableAmount = real
    ? Math.min(recoveryStake, liveBalance)
    : recoveryStake;
  ...
  useEffect(() => {
    for (const contract of openContracts) {
      ...
      if (profit < 0) {
        const nextStep = recoveryEnabled
          ? recoveryRef.current + 1
          : 0;

        recoveryRef.current = Math.min(
          nextStep,
          Number(maxRecoverySteps) || 0
        );
        ...
        setMessage(
          `LOSS · ${
            ...
            row?.barrier || analysis.barrier
          } closed · cooldown ${
            cooldownSeconds
          }s · searching for a fresh setup.`
        );
      } else {
        ...
        setMessage(
          "WIN · recovery reset · scanning for the next independent setup."
        );
      }
    }
  }, [
  ...
  useEffect(() => {
    ...
    const scanAndTrade = async () => {
      ...
      if (!targetMarkets.length) {
        setMessage(
          "SCANNING · waiting for Volatility 100/75/25/10 markets."
        );
        return;
      }

      const readyBooks = marketAnalyses.filter(
        (item) => item.digits.length >= 60
      );

      if (readyBooks.length < 1) {
        const samples = Math.max(
          ...marketAnalyses.map(
            (item) => item.digits.length
          ),
          0
        );

        setMessage(
          `BUILDING MARKET BOOKS · ${samples}/60 · loading V100/V75/V25/V10 in parallel.`
        );
      ...
      if (
        pnl <=
        -Math.abs(
          Number(stake) * Number(maxLosses)
        )
      ) {
        setRunning(false);

        setMessage(
          `SESSION STOP · ${money(pnl, currency)}`
        );
      ...
      if (real && !allowReal) {
        setMessage(
          "REAL selected · Arm REAL trading to permit execution."
        );
      ...
      if (real && liveBalance <= 0) {
        setMessage(
          "REAL waiting · live balance unavailable."
        );
      ...
      if (
        ...
        setMessage(
          "NO USABLE STAKE · waiting for a positive balance."
        );
      ...
      if (
        now - marketCycleRef.current >=
        10_000
      ) {
        marketCycleRef.current = now;

        setMessage(
          `SCANNING ALL · ${targetMarkets
            .map(
              (item) =>
      ...
      const usable = candidates.filter((a) => {
        ...
        if (
          candidateKey ===
            blockedKeyRef.current &&
          now < blockedUntilRef.current
        ) {
          return false;
        }

        const lastTradeEpoch = Number(
          lastTradeEpochBySymbolRef.current[
            a.symbol
          ] || 0
        );

        if (
          Number(a.latestEpoch || 0) <=
          lastTradeEpoch
        ) {
          return false;
        }
  ...
  const resetDigitBooks = () => {
    setBuffers({});
    setMessage(
      "60-digit books reset · rebuilding from live ticks."
    );
  };
  ...
  const currentSignal = signalValid
    ? `${analysis.direction} ${analysis.barrier}`
    : "WAIT";
  ...
  const payoutDisplay = lastDecision?.payout
    ? `${(
        (lastDecision.payout /
          Math.max(
            0.01,
  ...
  return (
    <section className="digitBot digitV10">
      <header className="digitHero">
        <div>
          <span>
            ZENTORA · DIGIT OVER / UNDER V12
          </span>

          <h2>
            Scan 4 Markets → Validate →
            Execute → Settle → Re-scan
          </h2>

          <p>
            V100, V75, V25 and V10 maintain
            separate 60-digit books and are
            evaluated continuously. The scanner
            rotates every 10 seconds for
            visibility but does not wait between
            scans.
          </p>
        </div>
        ...
        <div className="accountMode">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (
                targetId &&
                typeof auth.selectAccount ===
                  "function"
              ) {
                auth.selectAccount(targetId);
                setAllowReal(false);
              }
            }}
          >
      ...
      <div className="digitTopGrid v10grid">
        <div className="digitCard settingsCard">
          <div className="digitCardTitle">
            DIGIT OVER / UNDER SETTINGS
          </div>
          ...
          <button
            ...
            onClick={() =>
              setMessage(
                "Settings applied to the same DEMO/REAL strategy engine."
              )
            }
          >
            SAVE SETTINGS
        ...
        <div className="digitCard decisionCard">
          ...
          <div className="flowBox">
            <b>CALCULATE PROBABILITY</b>
            <small>
              OVER vs UNDER · live proposal ·
              expected value
            </small>
          </div>
          ...
          <div className="flowBox">
            <b>CHECK TRADE CONDITIONS</b>
            <small>
              {setupCount}/6 setup checks ·
              payout + EV · risk · cooldown
            </small>
          </div>

          <div className="flowArrow">↓</div>

          <div className="flowBox executeBox">
            <b>
              EXECUTE {currentSignal}
            </b>
            <small>
              Only after the live Deriv
              proposal passes
            </small>
          </div>
          ...
          <div className="lossBox">
            <b>IF LOSS</b>
            <span>
              Close the failed signal →
              cooldown → reject same key →
              re-analyze → find a fresh
              OVER/UNDER setup. Recovery is
              only used on a new validated
              setup.
            </span>
          </div>

          <div className="winBox">
            ✓ WIN → reset recovery → continue
            scanning
          </div>
        </div>
        ...
        <div className="digitCard signalCard">
          ...
          <div
            className={`bigSignal ${directionColor}`}
          >
            {currentSignal}
          </div>
          ...
          <div className="signalStats">
            ...
            <div>
              <small>Edge baseline</small>
              <b>
                {(
                  analysis.edgeVsBaseline *
                  100
                ).toFixed(1)}
                %
              </b>
          ...
          <div className="digitPath compactPath">
            {path.map((digit, i) => (
              <span
                key={`${i}-${digit}`}
                className={
                  digit > analysis.barrier
                    ? "over"
                    : "under"
                }
              >
          ...
          <button
            ...
            onClick={() =>
              setMessage(
                `SKIP SIGNAL · ${currentSignal} · searching next setup.`
              )
            }
          >
            ▸ SKIP SIGNAL
      ...
      <div className="digitLowerGrid v10lower">
        <div className="digitCard recoveryCard">
          ...
          <p className="digitNote">
            Recovery never forces the same
            losing signal. After a loss, that
            exact contract key is closed and
            blocked during cooldown. The next
            recovery stake can only be used
            after a fresh validated setup.
          </p>
        </div>
      ...
      <div className="digitActions">
        ...
        <button onClick={resetDigitBooks}>
          RESET 60-DIGIT BOOKS
        </button>
      ...
      <div className="digitCard digitJournal">
        <div className="digitCardTitle">
          DIGIT OVER / UNDER JOURNAL ·{" "}
          {tradeHistory.length}/8
        </div>

        {tradeHistory.length === 0 ? (
          <div className="digitEmpty">
            No executions yet. The engine will
            wait until a valid setup and
            positive Deriv proposal appear.
          </div>
        ) : (
          tradeHistory.map((row) => (
            <div
              className="digitJournalRow v10journal"
              key={row.id}
            >
              <b
                className={
      ...
      <footer className="digitFooter">
        <span>
          Markets: {targetMarkets.length}/4
        </span>
