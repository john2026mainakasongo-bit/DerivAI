...
export default function DigitOverRecoveryBot() {
  ...
  const [maxRecoverySteps, setMaxRecoverySteps] = useState(1);
  ...
  const [message, setMessage] = useState("Digit scanner ready â€” DEMO first.");
  ...
  const safeSelection = selection?.analysis && selection?.alternatives ? selection : emptySelection;
  ...
  useEffect(() => {
    for (const contract of openContracts) {
      ...
      if (p < 0) {
        const currentStep = Number(recoveryRef.current) || 0;

        if (!recoveryEnabled) {
          recoveryRef.current = 0;
          setRecoveryStep(0);
          setLosses((value) => value + 1);
          setMessage(`LOSS ${id} · recovery disabled · base stake`);
        } else if (currentStep === 0 && !recoveryExhaustedRef.current) {
          recoveryRef.current = 1;
          setRecoveryStep(1);
          setLosses((value) => value + 1);
          setMessage(`LOSS ${id} · recovery step 1/1`);
        } else {
          recoveryRef.current = 0;
          recoveryExhaustedRef.current = true;
          setRecoveryStep(0);
          setLosses((value) => value + 1);
          setMessage(`LOSS ${id} · recovery exhausted · base stake only`);
        }
      } else {
        recoveryRef.current = 0;
        recoveryExhaustedRef.current = false;
        setRecoveryStep(0);
        setMessage(`WIN ${id} Â· recovery reset`);
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      ...
      if (!a?.ready) {
        setMessage(`BUILDING 60-DIGIT WINDOW Â· ${a?.samples || 0}/60`);
        return;
      }
      if (pnl <= -Math.abs(Number(sessionStop))) {
        setRunning(false);
        setMessage(`SESSION STOP Â· ${money(pnl, currency)}`);
        return;
      }
      if (losses >= Number(maxLosses)) {
        setRunning(false);
        setMessage(`MAX LOSSES Â· ${losses}/${maxLosses}`);
        return;
      }
      if (real && !allowReal) {
        setMessage("REAL selected Â· Arm REAL trading to permit execution.");
        return;
      }
      if (real && liveBalance <= 0) {
        setMessage("REAL waiting Â· live balance unavailable. Reconnect or refresh the account balance.");
        return;
      }
      if (!Number.isFinite(safeAmount) || safeAmount <= 0) {
        setMessage("NO USABLE STAKE Â· waiting for a positive live balance.");
        return;
      }
      ...
      if (a.signal === "WAIT") {
        setMessage(`SKIP Â· ${a.regime} Â· model ${(a.probability * 100).toFixed(1)}% Â· ${a.reason}`);
        return;
      }
      ...
      try {
        setMessage(`PROPOSAL Â· OVER ${effectiveBarrier} Â· ${symbol} Â· ${args.duration}t Â· ${real ? "REAL" : "DEMO"}`);
        ...
        if (edge < Number(minEdge) || ev <= 0) {
          setMessage(`SKIP Â· proposal BE ${(implied * 100).toFixed(1)}% Â· model ${(a.probability * 100).toFixed(1)}% Â· edge ${(edge * 100).toFixed(1)}% Â· EV ${ev.toFixed(3)}`);
          return;
        }
        ...
        setMessage(`${real ? "REAL" : "DEMO"} EXECUTED Â· OVER ${effectiveBarrier} Â· C ${contractId || "accepted"}`);
      } catch (error) {
        setMessage(`EXECUTION FAILED Â· ${error instanceof Error ? error.message : String(error || tradeError || "unknown error")}`);
      } finally {
  ...
  const resetSession = () => {
    ...
    setTradeHistory([]);
    setMessage("Session reset â€” scanner ready.");
  };
  ...
  return (
    <section className="digitBot">
      <div className="digitHero">
        <div>
          <span>ZENTORA · DIGIT OVER RECOVERY V9.2</span>
        ...
        <div className="digitCard">
          ...
          <div className="digitControls">
            ...
            <label>Strategy<select value={barrier} onChange={(e) => setBarrier(e.target.value)}>
              <option value="AUTO">AUTO Â· BEST EDGE</option>
              <option value={2}>OVER 2</option>
              <option value={1}>OVER 1 · FALLBACK</option>
            </select></label>
          ...
          <div className="digitMarketLine"><span>Price</span><b>{Number(currentPrice || 0).toFixed(market?.decimals ?? 3)}</b><span>Last digit</span><strong>{currentDigits.at(-1) ?? "â€”"}</strong></div>
        </div>

        <div className="digitCard digitModelCard">
          <div className="digitCardTitle">AI DIGIT MAP Â· {symbol || "â€”"}</div>
          <div className="digitMetricGrid">
            ...
            <div><span>Hot digit</span><b>{analysis.hotDigit ?? "â€”"} Â· {analysis.hotCount || 0}Ã—</b></div>
            <div><span>Transition</span><b>{analysis.transition == null ? "â€”" : `${(analysis.transition * 100).toFixed(1)}%`}</b></div>
          ...
          <div className="digitReason">
            ...
            {barrier === "AUTO" && analysis.ready ? (
              <span className="digitAutoPick">
                AUTO PICK: OVER {effectiveBarrier} · O2 {((safeSelection.alternatives?.[2]?.probability ?? 0.5) * 100).toFixed(1)}% · O1 {((safeSelection.alternatives?.[1]?.probability ?? 0.5) * 100).toFixed(1)}%
              </span>
            ) : null}
          </div>
        </div>
      ...
      <div className="digitCard digitPathCard">
        <div className="digitCardTitle">CURSOR / DIGIT PATH Â· LAST 20 OF 60</div>
        <div className="digitPath">{analysis.path.length ? analysis.path.map((digit, index) => <span key={`${index}-${digit}`} className={digit > effectiveBarrier ? "over" : "under"}>{digit}</span>) : <em>Waiting for digitsâ€¦</em>}</div>
      ...
      <div className="digitLowerGrid">
        <div className="digitCard">
          ...
          <div className="digitControls compact">
            ...
            <label>Max recovery<input type="number" min="1" max="1" value={1} readOnly /></label>
          ...
          <p className="digitNote">{real ? "REAL LIVE BALANCE MODE Â· no artificial balance lock and no percentage cap. The requested stake uses the available live balance, then Deriv proposal decides acceptance." : "DEMO MODE."} V9.2: OVER 2 PRIMARY → OVER 1 FALLBACK. Recovery is limited to one step and never forces an entry. If the next scan has no clean setup, the bot skips it and immediately waits for the next opportunity.</p>
        </div>

        <div className="digitCard">
          <div className="digitCardTitle">PROPOSAL GATE</div>
          <div className="digitGate"><span>Contract</span><b>DIGITOVER Â· {effectiveBarrier}</b></div>
          <div className="digitGate"><span>Current key</span><b>{contractKey}</b></div>
          <div className="digitGate"><span>Selected account</span><b>{real ? "REAL" : "DEMO"} Â· {accountId || "NOT CONNECTED"}</b></div>
          <div className="digitGate"><span>Trading connection</span><b>{connected ? (real ? "REAL CONNECTED" : "DEMO CONNECTED") : "NOT CONNECTED"}</b></div>
          <div className="digitGate"><span>Real trading</span><b>{real ? (allowReal ? "ARMED Â· LIVE BALANCE" : "LOCKED Â· ARM REQUIRED") : "DEMO MODE"}</b></div>
          <div className="digitGate"><span>Execution</span><b>{active ? `OPEN Â· ${idOf(active)}` : "NO OPEN CONTRACT"}</b></div>
          <div className="digitMessage">{message}</div>
        </div>
      </div>

      <div className="digitActions">
        {!real && realAccount ? <button onClick={() => { auth.selectAccount(realAccount.id || realAccount.account_id || realAccount.loginid); setMessage("REAL account selected Â· waiting for authenticated trading connectionâ€¦"); }}>USE REAL ACCOUNT</button> : null}
      ...
      <div className="digitCard digitJournal">
        <div className="digitCardTitle">DIGIT OVER JOURNAL Â· {tradeHistory.length}/8</div>
        {tradeHistory.length === 0 ? <div className="digitEmpty">No executions yet. Scanner will skip until a clean setup + positive Deriv proposal appears.</div> : tradeHistory.map((row) => <div className="digitJournalRow" key={row.id}><b className={row.result === "WON" ? "profit" : row.result === "LOST" ? "loss" : "open"}>{row.result}</b><span>OVER {row.barrier}</span><span>{row.symbol}</span><span>{row.duration}t</span><span>Model {(Number(row.model || 0) * 100).toFixed(1)}%</span><span>Edge {(Number(row.edge || 0) * 100).toFixed(1)}%</span><span>{row.pnl == null ? "â€”" : money(row.pnl, currency)}</span></div>)}
      </div>

      <div className="digitFooter"><span>Markets: {Object.keys(buffers).length}</span><span>Current 60: {currentDigits.length}/60</span><span>Strategy: {barrier === "AUTO" ? `AUTO â†’ OVER ${effectiveBarrier}` : `OVER ${effectiveBarrier}`}</span><span>Last scan: {lastScanAt ? new Date(lastScanAt).toLocaleTimeString() : "â€”"}</span><span className="spacer">{real ? "REAL" : "DEMO"} Â· {currency}</span></div>
    </section>
  );
}
