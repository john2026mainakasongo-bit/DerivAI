...
const STORAGE_KEY="zentora_adaptive_trend_deriv_v8_performance";
...
export default function AdaptiveTrendBot(){
 ...
 const requestedStake=Math.max(.35,Number(stake)||.35);

 useEffect(()=>{try{localStorage.setItem(STORAGE_KEY,JSON.stringify({version:8,source:"deriv",updatedAt:Date.now(),performance}))}catch{}},[performance]);
   ...
   try{
     ...
     for(let attempt=1;attempt<=2;attempt++){
       try{
         ...
         if(real&&amount>balance*.01){setMessage(`REAL RISK LOCK · requested stake ${money(amount,currency)} exceeds 1% of balance. DEMO/REAL strategy remains identical; reduce the stake to trade this account.`);busy.current=false;return;}
 ...
 },[active,allowReal,balance,connected,contractType,cooldown,currency,direction,duration,horizon,keyStats.samples,keyStats.winRate,losses,maxLosses,minEdge,pnl,quoteTrade,placeQuotedTrade,real,requestedStake,running,sl,symbol,tp,tradeBusy,tradeError,tradingAccountId,strictRealEvidence]);
 ...
 return <section className="adaptiveBot">
  <div className="adaptiveHeader"><div><span className="adaptiveEyebrow">ZENTORA - ADAPTIVE TREND V8</span><h2>Deriv → Regime → Evidence → Proposal → Execution</h2><p>One strategy engine for DEMO + REAL · payout-aware EV · exact-key evidence for REAL unlock.</p></div><div className={`adaptiveRunState ${running?"on":""}`}>{running?"SCANNING":"STOPPED"}</div></div>
  ...
  <div className="adaptiveActionBar"><div><b>{active?`DERIV TRADE OPEN: ${idOf(active)}`:executionState==="OPENED"?"DERIV TRADE OPENED":executionState==="WON"?"LAST DERIV TRADE: WON":executionState==="LOST"?"LAST DERIV TRADE: LOST":analysis.signal==="WAIT"?"WAITING FOR DERIV EVIDENCE":`READY: ${analysis.signal}`}</b><small>{message}</small></div><div className="adaptiveActions">
   ...
   <button className="secondary" onClick={reset}>RESET</button><button className="secondary" onClick={clearEvidence}>RESET EVIDENCE</button><button className={running?"danger":"primary"} onClick={()=>{if(running){setRunning(false);setMessage("Deriv bot stopped. Protection remains active.")}else{setRunning(true);setMessage(real?(allowReal?"REAL Deriv scanner armed · same V8 signal/entry engine; REAL authorization gate active.":"REAL selected but not armed."):"DEMO Deriv scanner armed · same V8 signal/entry engine as REAL.")}}}>{running?"STOP BOT":"START BOT"}</button>
  </div></div>
