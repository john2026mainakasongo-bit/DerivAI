...
export function TouchNoTouchBotView({ feed }) {
  const { markets = [], market, symbol, connected, authenticatedFeed, status, ticks = [], prices = [], currentPrice, openContracts = [], transactions = [], changeSymbol, quoteTrade, placeQuotedTrade, getContractsFor, selectedAccount, selectedAccountType, selectedAccountId, tradeBusy } = feed;
  ...
  const [contractAvailability, setContractAvailability] = useState({
    ...
    touch: false,
    noTouch: false,
    message: "Checking Touch / No Touch availability…",
  });
  ...
  useEffect(() => {
    if (!connected || !symbol || !getContractsFor) return;

    const requestId = availabilityRequestRef.current + 1;
    availabilityRequestRef.current = requestId;

    setContractAvailability((current) => ({
      ...current,
      checked: false,
      message: "Checking available Touch / No Touch contracts…",
    }));

    void getContractsFor(symbol)
      .then((response) => {
        if (requestId !== availabilityRequestRef.current) return;

        const available =
          response?.contracts_for?.available ||
          response?.data?.contracts_for?.available ||
          response?.data?.available ||
          [];

        const types = new Set(
          available.map((item) =>
            String(item?.contract_type || "").toUpperCase()
          )
        );

        const touch = types.has("ONETOUCH");
        const noTouch = types.has("NOTOUCH");
        ...
        setContractAvailability({
          ...
          message:
            touch && noTouch
              ? "ONETOUCH + NOTOUCH available"
              : touch
                ? "ONETOUCH available; NOTOUCH unavailable"
                : noTouch
                  ? "NOTOUCH available; ONETOUCH unavailable"
                  : "Touch / No Touch is not available for this market",
        });
      })
      .catch((error) => {
        if (requestId !== availabilityRequestRef.current) return;
        ...
        setContractAvailability({
          ...
          message:
            error instanceof Error
              ? `Contract availability check failed: ${error.message}`
              : "Contract availability check failed.",
        });
      });
  }, [connected, getContractsFor, symbol]);
  ...
  const checkProposal = useCallback(async (requestedSide) => {
    ...
    if (
      !contractAvailability.checked
    ) {
      setQuoteError("Checking contract availability for this market…");
      return;
    }

    if (
      (!contractAvailability.touch && setup === "TOUCH") ||
      (!contractAvailability.noTouch && setup === "NO TOUCH")
    ) {
      setQuoteError(
        `${setup} is not available for ${symbol}. Choose a market that supports this contract.`
      );
      setMessage(`${setup} is not available for this market.`);
      return;
    }
    ...
    setMessage(`TESTING ${setup} Â· ${contractType} PROPOSAL — NO BUY…`);
  ...
  const execute = useCallback(async (mode = "AUTO", forcedAnalysis = analysis, forcedStake = null) => {
    ...
    if (
      !contractAvailability.checked ||
      !contractAvailability.touch ||
      !contractAvailability.noTouch
    ) {
      setMessage(
        contractAvailability.checked
          ? "Touch / No Touch is unavailable for this market."
          : "Checking Touch / No Touch availability…"
      );
      return;
    }
  ...
  }, [allowReal, analysis, balance, barrierMultiplier, contractAvailability, currency, currentPrice, duration, fixedStake, minScore, placeQuotedTrade, quoteTrade, selectedAccountId, selectedAccountType, sessionPnl, sessionStop, sessionTarget, stakeMode, symbol, trades]);
  ...
  useEffect(() => {
    for (const c of touchContracts) {
      ...
      setMessage(`${won ? "âœ“ WIN" : "âœ• LOSS"} • ${typeOf(c)} • ${pnl >= 0 ? "+" : ""}${money(pnl, currency)}`);
    }
  ...
  return (
    <section className="tntShell">
      ...
      <div className={`tntAvailability ${contractAvailability.touch && contractAvailability.noTouch ? "ready" : "blocked"}`}>
        <span>CONTRACT AVAILABILITY</span>
        <strong>{contractAvailability.message}</strong>
        {contractAvailability.checked && (!contractAvailability.touch || !contractAvailability.noTouch) ? (
          <small>Choose another market before trading this desk.</small>
        ) : null}
      </div>
