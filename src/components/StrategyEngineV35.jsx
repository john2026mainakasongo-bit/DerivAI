  ...
  return (
    <div className="v30Stepper">
      ...
      <div className="v30StepButtons">
        <button type="button" disabled={disabled || safe <= min} onClick={() => change(-step)}>ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢</button>
  ...
  return (
    <div className="v30Shell">
      <aside className="v30Sidebar">
        <div className="v30Brand"><div className="v30BrandMark">ÃƒÂ¢Ã…â€œÃ‚Â¦</div><div><strong>Deriv<span>AI</span></strong><small>Strategy Engine</small></div></div>
        <div className="v30LivePill"><i /> DERIV LIVE</div>
        <nav className="v30Nav">
          <a className="active" href="#engine">ÃƒÂ¢Ã…â€™Ã¢â‚¬Å¡ <span>Strategy Engine</span><b>LIVE</b></a>
          <a href="#transactions">ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â£ <span>Bot Transactions</span></a>
          <a href="#performance">ÃƒÂ¢Ã¢â‚¬â€Ã¢â‚¬Â <span>Performance</span></a>
          <a href="#settings">ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢ <span>Settings</span></a>
        </nav>
        <div className="v30BotCard">
          <div className="v30BotTop"><span>BOT STATUS</span><b className={running ? "on" : ""}><i />{running ? "ACTIVE" : "READY"}</b></div>
          <div className="v30BotOrb">ÃƒÂ¢Ã¢â‚¬â€Ã¢â‚¬Â°</div>
        ...
        <div className="v30Footer">Ãƒâ€šÃ‚Â© 2026 DerivAI<br />All rights reserved.</div>
      </aside>
      ...
      <main className="v30Main" id="engine">
        <header className="v30Topbar">
          <div className="v30TopLeft">
            ...
            <span className={`v30Connection ${connected ? "connected" : "error"}`}><i />{connected ? "CONNECTED" : (loadingMarket ? "CONNECTINGÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : status || "DISCONNECTED")}</span>
          </div>
          <div className="v30TopRight">
            <span className="v30Server">Server Time<strong>{new Date().toLocaleTimeString()}</strong></span>
            <span className="v30Icon">ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢</span><span className="v30Icon">ÃƒÂ¢Ã‹Å“Ã‚Â¾</span>
              ...
              >
                {accountOptions.length ? accountOptions.map((account) => (
                  <option key={account.id} value={account.id}>
                    {account.type === "demo" || account.type === "virtual" ? "DEMO" : "REAL"} Ãƒâ€šÃ‚Â· {account.label} Ãƒâ€šÃ‚Â· {Number(account.balance || 0).toFixed(2)} {account.currency}
                  </option>
                )) : <option value="">{auth.authenticated ? "No Deriv accounts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Refresh" : "Connect Deriv Account"}</option>}
              </select>
              <small>{!auth.authenticated ? "Login required" : isDemo ? "Demo / VRTC Ãƒâ€šÃ‚Â· execution ready" : "REAL selected Ãƒâ€šÃ‚Â· bot locked"}</small>
            </div>
            <button
            ...
            >{auth.authenticated ? "ÃƒÂ¢Ã¢â‚¬Â Ã‚Âª" : "ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â€"}</button>
          </div>
        </header>

        <section className="v30Hero">
          <div><div className="v30Eyebrow">CALIBRATED STRATEGY ENGINE Ãƒâ€šÃ‚Â· V35</div><h1>Deriv Strategy Engine</h1><p>Live analysis Ãƒâ€šÃ‚Â· Smart signals Ãƒâ€šÃ‚Â· Real results</p></div>
          <div className={`v30Feed ${connected ? "on" : ""}`}>
            <i />{connected ? `Live feed Ãƒâ€šÃ‚Â· ${effectiveSymbol}` : "Deriv feed offline"}
        ...
        <section className="v30Summary">
          <article className="v30Card signalHero"><div className="radar">ÃƒÂ¢Ã¢â‚¬â€Ã…Â½</div><div><span>CURRENT SIGNAL</span><strong>{signal}</strong><small>Confidence <b>{confidence.toFixed(1)}%</b></small><small className="rapidGateText">RAPID GATE: <b>{rapidGateStatus}</b></small></div></article>
          <article className="v30Card"><span>CURRENT DIGIT</span><div className="v30BigDigit">{lastDigit ?? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</div><Sparkline values={(digitHistory || []).slice(-50)} /></article>
          ...
          <article className="v30Card riskCard"><span>RISK MODE</span><strong>FIXED / NO</strong><small>MARTINGALE</small><b>ÃƒÂ¢Ã¢â‚¬â€Ã‹â€ </b></article>
        </section>
        ...
        <section className="v30Workspace">
          <div className="v30Center">
            <article className="v30Card v30ChartCard">
              <div className="v30CardHead"><div><span>LIVE MARKET FEED <em>ÃƒÂ¢Ã¢â‚¬â€Ã‚Â LIVE</em></span><strong>{Number.isFinite(currentPrice) ? currentPrice.toFixed(market?.decimals ?? 2) : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</strong></div><div className="lastDigitBadge"><b>{lastDigit ?? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</b><small>Last Digit</small></div></div>
          ...
          <aside className="v30Right">
            <article className="v30Card tradeCard" id="settings">
              <div className="v30PurpleHead"><span>ÃƒÂ¢Ã¢â‚¬â€Ã¢â‚¬Â° TRADE CONTROL</span><small>{isDemo ? "DEMO SAFE" : "LOCKED"}</small></div>
              ...
              <label>Entry Analysis<select value={settings.analysisTimeframe} disabled={busy} onChange={(e) => setNumber("analysisTimeframe", Number(e.target.value))}><option value="30">30 Seconds Ãƒâ€šÃ‚Â· RAPID</option><option value="60">1 Minute Ãƒâ€šÃ‚Â· DEEP</option></select></label><label>Contract Duration<select value={settings.duration} disabled={busy} onChange={(e) => setNumber("duration", Number(e.target.value))}><option value="1">1 Tick (1s)</option><option value="2">2 Ticks (2s)</option><option value="3">3 Ticks (3s)</option><option value="5">5 Ticks (5s)</option></select></label>
              <button
                className="v30Start"
                type="button"
                disabled={busy || !auth.authenticated || !selectedId || !isDemo || !connected || historySize < 18 || accountBusy}
                onClick={startBot}
              >ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¶ {busy ? "BOT RUNNING" : executionReady ? "START DEMO BOT" : "CONNECT & START DEMO"}</button>
              <button className="v30Stop" type="button" disabled={!busy} onClick={stopBot}>ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â  STOP BOT</button>
                  ...
                  : !selectedId
                    ...
                    : !isDemo
                      ...
                      : !connected
                        ...
                        : historySize < 18
                          ? `Calibrating rapid entryÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ ${historySize}/18 ticks.`
                          : !executionReady
                            ? "Authenticated demo trading connection is being preparedÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦"
                            : botState.message}
              </div>
            </article>

            <article className="v30Card perfCard" id="performance"><div className="v30PurpleHead"><span>ÃƒÂ¢Ã¢â‚¬â€Ã¢â‚¬Â° PERFORMANCE SUMMARY</span></div><div className="v30PerfBody"><div className="v30Donut" style={{ "--p": `${Math.min(100, winRate)}%` }}><strong>{winRate.toFixed(0)}%</strong><small>Win Rate</small></div><div className="v30PerfStats"><span>Total Signals<b>{botState.runs}</b></span><span>Total Profit<b className="green">+{totalProfit.toFixed(2)} USD</b></span><span>Total Loss<b className="red">-{totalLoss.toFixed(2)} USD</b></span><span>Net Profit<b className={netProfit >= 0 ? "green" : "red"}>{netProfit >= 0 ? "+" : ""}{netProfit.toFixed(2)} USD</b></span></div></div><div className="v30Excellent">ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â€ Performance: {winRate >= 70 ? "EXCELLENT" : winRate >= 50 ? "STABLE" : "LEARNING"}</div></article>
          </aside>
        </section>

        <section className="v30Card transactions" id="transactions"><div className="v30TxHead"><div><span>ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â£ BOT TRANSACTION VIEW</span><small>Live execution ledger</small></div><select value={filter} onChange={(e) => setFilter(e.target.value)}><option value="ALL">All Transactions</option><option value="WIN">Wins</option><option value="LOSS">Losses</option></select></div><div className="v30TableWrap"><table><thead><tr><th>ID</th><th>TIME</th><th>TYPE</th><th>DURATION</th><th>AMOUNT</th><th>ENTRY</th><th>EXIT</th><th>RESULT</th><th>PROFIT/LOSS</th><th>STATUS</th></tr></thead><tbody>{transactions.length ? transactions.slice(0, 8).map((item, index) => <tr key={`${item.id || index}-${item.time || index}`}><td>#{item.id || 1000 + index}</td><td>{item.time ? new Date(item.time).toLocaleTimeString() : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td><td><b className="txType">{item.setup || botState.activeSetup || "DIGIT"}</b></td><td>{settings.duration} Tick</td><td>{Number(item.stake ?? settings.stake).toFixed(2)}</td><td>{item.entrySpot ?? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td><td>{item.exitSpot ?? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td><td><b className={String(item.result).toLowerCase() === "win" ? "txWin" : "txLoss"}>{item.result || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</b></td><td className={Number(item.profit) >= 0 ? "txWin" : "txLoss"}>{Number(item.profit) >= 0 ? "+" : ""}{Number(item.profit || 0).toFixed(2)}</td><td><span className="settled">Settled</span></td></tr>) : <tr><td colSpan="10" className="v30NoTx">No transactions yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â start the Demo Bot to populate this view.</td></tr>}</tbody></table></div><button className="v30AllTx" type="button">ÃƒÂ¢Ã‹Å“Ã‚Â· View All Transactions</button></section>
      </main>
    </div>
  );
}
