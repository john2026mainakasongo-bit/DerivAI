...
import "../styles/StrategyEngineV30.css";
...
export default function StrategyEngineV30() {
  ...
  async function startBot() {
    ...
    if (!isDemo) {
      setConnectionError("Select a DEMO account before starting the bot. Real-money execution remains locked.");
      return;
    }
    if (!effectiveSymbol) {
    ...
    if (!connected) {
      try {
        ...
        setConnectionError(error?.message || "Deriv feed connection failed.");
        return;
      }
    }
  ...
  const accountOptions = useMemo(() => {
    ...
    return list.filter((a) => a && accountId(a)).map((a) => ({
      id: accountId(a),
      type: String(a.type || a.account_type || (String(a.loginid || a.id || "").startsWith("VRTC") ? "demo" : "real")).toLowerCase(),
      currency: a.currency || "USD",
      balance: Number(a.balance ?? 0),
      label: a.loginid || a.id || a.account_id || "Account",
    }));
  }, [auth.accounts, auth.availableAccounts, auth.accountList, auth.user?.accounts]);

  function selectAccount(id) {
    const fn = auth.selectAccount || auth.switchAccount || auth.setSelectedAccount || auth.setAccount;
    if (typeof fn === "function") {
      try { fn(id); setConnectionError(""); }
      catch (error) { setConnectionError(error?.message || "Unable to switch account."); }
    } else {
      setConnectionError("Your auth provider does not expose account switching in this build.");
    }
  }
      ...
      <main className="v28Main" id="engine">
        <header className="v28Topbar">
          <div className="v28TopLeft">
            ...
            <span className={`v28Connection ${connected ? "connected" : "error"}`}><i />{connected ? "CONNECTED" : (loadingMarket ? "CONNECTING…" : status || "DISCONNECTED")}</span>
          </div>
        ...
        <section className="v28Hero">
          <div><div className="v28Eyebrow">CALIBRATED STRATEGY ENGINE · V30</div><h1>Deriv Strategy Engine</h1><p>Live analysis · Smart signals · Real results</p></div>
          <div className={`v28Feed ${connected ? "on" : ""}`}>
        ...
        <section className="v28Workspace">
          ...
          <aside className="v28Right">
            <article className="v28Card tradeCard" id="settings">
              ...
              <button className="v28Start" type="button" disabled={busy || !isDemo || !connected || historySize < 8} onClick={startBot}>▶ START BOT</button>
              <button className="v28Stop" type="button" disabled={!busy} onClick={stopBot}>■ STOP BOT</button>
              <div className={`v28BotReady ${connected && historySize >= 8 && signal !== "WAIT" ? "ready" : ""}`}><i />{!isDemo ? "Demo account required for bot execution." : botState.message}</div>
            </article>
