...
export default class DerivBotEngine {
  constructor({ client, onState }) {
    ...
    this.state = {
      ...
      blockedSetupUntil: 0,
      lastLossSetup: "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â",
      ...
      gate: null,
      executionPhase: "SCAN",
      lockedCandidate: "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â",
      signalVersion: 0,
    ...
    if (!selected || !selectedCanExecute) {
      ...
      return {
        ...
        gate: {
          ...
          executionPhase: "SCAN",
          lockedCandidate: selected?.setup || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â",
          signalVersion: this.signalVersion,
          lockedSignalVersion: 0,
        },
      };
  ...
  async start() {
    ...
    this.patch({
      status: "RUNNING",
      message:
        "V49 keeps Demo digits-only, adds lower/upper row pressure, ranks OVER/UNDER barriers 1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“7, dynamically ranks DIFFERS digits, and enters after one fresh confirmation.",
      scanStartedAt: Date.now(),
      scanElapsedSeconds: 0,
      scanTicks: 0,
      maxScanTicks: this.settings.maxScanTicks,
  ...
  async loop() {
    while (this.running && !this.stopping) {
      ...
      console.debug("[V49] GATE", {
        ...
        lockedCandidate: this.lockedCandidate?.setup || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â",
      ...
      if (!check.ok) {
        this.patch({
          ...
          lockedCandidate: check.gate?.lockedCandidate || this.lockedCandidate?.setup || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â",
        });
      ...
      try {
        ...
        this.addHistory({
          id: `error-${Date.now()}`,
          time: Date.now(),
          setup: `${check.contract.label} Ãƒâ€šÃ‚Â· ${message}`,
          result: "ERROR",
          profit: 0,
  ...
  async executeTrade(check) {
    ...
    this.patch({
      ...
      message:
        `${this.isDemoAccount ? "Demo" : "REAL"} Ãƒâ€šÃ‚Â· V49 fast digit-row entry confirmed ${check.contract.label} at scan tick ${this.scanTickCount}/${this.settings.maxScanTicks} for ${durationText(
          tradeDuration,
          tradeDurationUnit
        )}. Requesting ${stake.toFixed(2)} ${this.currency}.`,
      activeSetup: `${check.contract.label} Ãƒâ€šÃ‚Â· V49 FAST DIGIT ROW`,
    ...
    this.addHistory({
      ...
      riskLevel: String(check.decision.riskLevel || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"),
      entryStage: String(check.timing.state || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"),
    ...
    this.patch({
      ...
      blockedSetupUntil,
      lastLossSetup: this.lastLossSetup || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â",
      lossProtectionCount,
      lastBlockReason: won
        ? ""
